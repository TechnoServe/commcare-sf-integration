...
def process_farm(data: dict, sf_connection):
    ...
    fis_data = data.get("form", {}).get("field_inventory_survey", "")
...
def generate_farm_fields(data: dict, farm: dict, request_id, url_string):
    return {
        ...
        "Main_Coffee_Field__c": True if farm.get("best_practice_plot", "") == "1" else False,
        "Planted_on_visit_date__c": True if farm.get("important_notes_planting_dates", {}).get("planting_period_note_same_date_as_visit", "") == "yes" else False,
        "Planted_out_of_season__c": True if farm.get("important_notes_planting_dates", {}).get("planting_period_note_out_of_season", "") == "yes" else False,
        "Planted_out_of_season_comments__c": farm.get("important_notes_planting_dates", {}).get("planting_period_comment_out_of_season", "") or "",
